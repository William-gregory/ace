...
class OneStepAggregator(AggregatorABC[CoupledTrainOutput]):
    ...
    def get_summary(self, label: str) -> AggregatorSummary:
        ...
        if self._aggregators["ice"] is not None:
            ice_summary = self._aggregators["ice"].get_summary(label)
            ice_logs = dict(ice_summary.logs)
            ice_logs.pop(f"{label}/mean/loss", None)
        ...
        if self._aggregators["atmosphere"] is None:
            ...
            duplicates = set(ocean_logs.keys()) & set(atmos_logs.keys()) & set(ice_logs.keys())
            if len(duplicates) > 0:
                raise ValueError(
                    "Duplicate keys found in ocean, atmosphere, and ice"
                    f"{label} logs: {duplicates}."
                )
            
            logs = {**ocean_logs, **atmos_logs, **ice_logs}
            logs[f"{label}/mean/loss"] = loss
            loss_ocean = self._loss_ocean / self._n_batches
            logs[f"{label}/mean/loss/ocean"] = float(
                self._dist.reduce_mean(loss_ocean.detach()).cpu().numpy()
...
class InferenceEvaluatorAggregator(
    ...
    def get_summary(self) -> InferenceSummary:
        ...
        if self.ice is not None:
            ice_summary = self.ice.get_summary()
            ice_logs = dict(ice_summary.logs)
        ...
        if self.atmosphere is None:
            ...
            if len(duplicates) > 0:
                raise ValueError(
                    "Duplicate keys found in ocean and atmosphjere"
                    f"inference evaluator aggregator logs: {duplicates}."
                )
            logs = {
                "time_mean_norm/rmse/channel_mean": channel_mean,
            ...
            duplicates = set(atmos_logs.keys()) & set(ice_logs.keys())
            if len(duplicates) > 0:
                raise ValueError(
                    "Duplicate keys found in atmosphere and ice"
                    f"inference evaluator aggregator logs: {duplicates}."
                )
            logs = {
                "time_mean_norm/rmse/channel_mean": channel_mean,
            ...
            channel_mean = (
                ...
                self._num_channels_ocean
                + self._num_channels_ice
                + self._num_channels_atmos
            )
...
class InferenceAggregator(
    ...
    def get_summary(self) -> InferenceSummary:
        ...
        ice_summary = self.ice.get_summary()
        atmos_summary = self.atmosphere.get_summary()
        duplicates = set(ocean_summary.logs.keys()) & set(ice_summary.logs.keys()) & set(atmos_summary.logs.keys())
        if len(duplicates) > 0:
            raise ValueError(
        ...
        return InferenceSummary(
            logs={**ocean_summary.logs, **atmos_summary.logs, **ice_summary.logs}, loss=None
        )
