...
class StandaloneComponentCheckpointsConfig:
    ...
    def load_stepper_config(self) -> CoupledStepperConfig:
        ...
        if self.ocean is not None:
            ocean_config=ComponentConfig(
                ...
                stepper=load_single_stepper_config(
                    self.ocean.pathl, self.ocean_stepper_override
                ),
            )
        if self.ice is not None:
            ice_config=ComponentConfig(
                ...
                stepper=load_single_stepper_config(
                    self.ice.path, self.ice_stepper_override
                ),
            )
    ...
    def load_stepper(self) -> CoupledStepper:
        ...
        if self.ice is not None:
            ice = load_single_stepper(self.ice.path, self.ice_stepper_override)
            ice_data = ice.training_dataset_info
        if self.atmosphere is not None:
            atmos = load_single_stepper(self.atmosphere.path, self.atmosphere_stepper_override)
