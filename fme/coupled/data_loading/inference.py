...
class InferenceDataLoaderConfig:
    ...
    def __post_init__(self):
        ...
        if self.dataset.atmosphere is not None and self.dataset.atmosphere.subset != Slice(
            None, None, None
        ):
            raise ValueError(
...
class InferenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if config.dataset.atmosphere is None:
            ...
            config.dataset.atmosphere.update_subset(TimeSlice(start_time=ice.first_time))
            atmosphere, atmosphere_properties = config.dataset.atmosphere.build(
                atmosphere_reqs.names, atmosphere_reqs.n_timesteps_schedule
            )
        else:
    ...
    def _get_batch_data(self, index) -> CoupledBatchData:
        ...
        if self._dataset._atmosphere is not None:
            atmosphere_dims = list(self.properties.horizontal_coordinates.atmosphere.dims)
